...
* **Amiibo ID:** A 16-character hex string (8 bytes) that defines the character identity (e.g., Link, Mario, Mineru).
...
* **Leaked Data Support:** Includes an `extras.json` file to support leaked or unreleased Amiibos (like Mineru's Construct) not yet available in standard APIs.
...
The files generated by this tool are **unencrypted**. Most physical hardware and official emulators require these files to be encrypted using Nintendo's proprietary "Retail Keys" (`fixed-info.bin` and `locked-secret.bin`).
...
* **AmiiboAPI:** The primary source for character metadata.
* **NFC-Bank Research:** For the original NTAG215 reverse engineering documentation.
